...
export interface ChatCompletion { id: string; choices: { message: { role: string; content: string }; finish_reason: string; }[]; usage: { prompt_tokens: number; completion_tokens: number; total_tokens: number; }; }
...
class LLMClient {
    ...
    async chat(messages: ChatMessage[], opts?: { temperature?: number; max_tokens?: number }): Promise<ChatCompletion> {
        if (!this.chatClient || !this.config) throw new Error('LLM client not configured. Please set API config in settings.');
        return this.withRetry(async () => {
            const response = await this.chatClient!.post('/chat/completions', {
